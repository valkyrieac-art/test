...
create index if not exists activities_search_idx on public.activities using gin (to_tsvector('simple', title || ' ' || place || ' ' || content));
...
create trigger budgets_keep_single_active
after insert or update of is_active on public.budgets
for each row execute function public.keep_single_active_budget();
