import type { Activity, ActivityInput, Budget, BudgetInput, Expense, ExpenseInput } from '../types';
...
export const demoStore = {
  ...
  updateActivity(id: string, input: ActivityInput) {
    ...
    write(
      ACTIVITY_KEY,
      activities.map((item) => (item.id === id ? updated : item)),
    );
    return updated;
  },
  ...
  deleteActivity(id: string) {
    write(
      ACTIVITY_KEY,
      read<Activity[]>(ACTIVITY_KEY, seedActivities).filter((item) => item.id !== id),
    );
  },
  ...
  updateExpense(id: string, input: ExpenseInput) {
    ...
    write(
      EXPENSE_KEY,
      expenses.map((item) => (item.id === id ? updated : item)),
    );
    return updated;
  },
  ...
  deleteExpense(id: string) {
    write(
      EXPENSE_KEY,
      read<Expense[]>(EXPENSE_KEY, seedExpenses).filter((item) => item.id !== id),
    );
  },
